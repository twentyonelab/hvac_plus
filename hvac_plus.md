...
- **Cofnij / przywróć na jednym stosie, 100 kroków wstecz.** `snapshot()` czyści stos „przywróć", `undo()` i `redo()` przerzucają stany między stosami. Każda zmiana na planie przechodzi przez `snapshot()`, więc historia obejmuje też przeciągnięte karty i automatykę. Podkłady (dataURL, nierzadko kilka MB) nie wędrują do historii — zrzut trzyma tylko odnośnik do puli obrazów, więc setka kroków waży tyle, co sam projekt, a cofnięcie „wgraj podkład” naprawdę zdejmuje podkład. Przyciski są w dwóch miejscach: nad rzutem (z licznikiem zapamiętanych kroków) i w szynie narzędzi rysunku.
- **Automatyczne łączenie sieci jest idempotentne.** `autoConnect()` dodaje tylko brakujące odcinki — istniejące trasy i ręcznie poprowadzone załamania zostają nietknięte, więc przycisk można nacisnąć po każdej zmianie. Nawiew i wywiew to dwie niezależne gałęzie (osobne rozdzielacze), a kondygnacje bez centrali wiąże pion o ustalonym numerze (1 = nawiew, 2 = wywiew). Brakujące rozdzielacze stawiane są w holu albo w pomieszczeniu technicznym, bo tam realnie wiszą.
